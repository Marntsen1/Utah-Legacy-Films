        ...
        <div className="p-8">
          ...
          {step === 1 ? (
            <form onSubmit={(e) => { e.preventDefault(); setStep(2); }} className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-[#362b24] uppercase tracking-wide mb-1.5">Full Name</label>
                <input required type="text" className="w-full bg-white border border-[#362b24]/10 rounded-xl px-4 py-3 text-[#362b24] focus:outline-none focus:border-[#c06e46] transition-colors" placeholder="Jane Doe" />
              </div>
              <div>
                <label className="block text-xs font-medium text-[#362b24] uppercase tracking-wide mb-1.5">Email Address</label>
                <input required type="email" className="w-full bg-white border border-[#362b24]/10 rounded-xl px-4 py-3 text-[#362b24] focus:outline-none focus:border-[#c06e46] transition-colors" placeholder="jane@example.com" />
              </div>
              <div>
                <label className="block text-xs font-medium text-[#362b24] uppercase tracking-wide mb-1.5">Preferred Month</label>
                <select className="w-full bg-white border border-[#362b24]/10 rounded-xl px-4 py-3 text-[#362b24] focus:outline-none focus:border-[#c06e46] transition-colors">
  ...
  const getTheme = (id: string) => {
    switch (id) {
        ...
        case 'signature': // Reverted to Dark Brown with explicitly defined corners
            return {
                card: "bg-[#362b24] text-[#f5f2eb] rounded-3xl overflow-hidden shadow-2xl shadow-black/20 scale-105 z-10 border border-[#4a3e36] hover:-translate-y-2",
                title: "text-[#f5f2eb]",
                price: "text-[#f5f2eb]",
                desc: "text-[#a89b91]",
    ...
    <section id="plans" className="relative py-24 md:py-32 px-6">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-center">
          {packages.map((pkg, i) => {
            const theme = getTheme(pkg.id) as any;
